...
metadata = {
    'id': 'method_rdkit_fcfp4_1023_tanimoto',
    'representation': 'fcfp4_1023',
    'similarity': 'tanimoto'
}
...
def create_model(ligands, decoys):
    ...
    """
    return [AllChem.GetMorganFingerprintAsBitVect(molecule, 2, nBits=1023,
                                                  useFeatures=True)
...
def compute_score(model, query):
    ...
    """
    fingerprint = AllChem.GetMorganFingerprintAsBitVect(query, 2, nBits=1023,
                                                        useFeatures=True)
