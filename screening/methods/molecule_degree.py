...
metadata = {'name': 'molecule_size'}


# region Similarity method implementation

def _create_model(actives, inactives):
    model = {'actives': actives}
    return model


def _compute_score(model, query):
    distance = min(
        [abs(query.GetNumAtoms() - active.GetNumAtoms())
         for active in model['actives']])
    return 1.0 / distance if distance != 0.0 else 2.0
